...
def epsilon_greedy(state_vector, epsilon):
    ...
    if np.random.random() > epsilon:
        ...
        object_index = np.random.choice(np.arange(NUM_OBJECTS))
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    q_value = 1 / 2 * (q_values_action_cur[action_index]
                     + q_values_object_cur[object_index])
    loss = 1/2*(reward+(not terminal)*GAMMA*maxq_next-q_value)**2
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
